 ...
 * 3. Let S=B+A be the seeds, generate the graph dataset D;
 * 4. To generate a graph, pick a seed  and merge it into the graph until the
 *    graph reaches a Poisson distributed size.
 *
...
public class GenGraph {
  ...
  /**
   * Check if i in V1 can be merged with j in V2
   */
  private static boolean checkPair(int[] V1, int[][] adj1, int n1, int[] V2,
  ...
  public static LGraph mergeGraphs(LGraph g1, LGraph g2) {
    ...
    for (int i = 0; i < n1; i++) {
      for (int j = 0; j < n2; j++) {
        // Check if i and j can be overlapped
        if (checkPair(g1.V, adj1, n1, g2.V, adj2, n2, map, rmap, i, j)) {
          map[i] = j;
          rmap[j] = i;
          cnt++;
          break;
  ...
  public static LGraph[] genGraphs(LGraph[] seeds, double[] accuProb, int size,
    ...
    for (int i = 0; i < num; i++) {
      ...
      graphs[i] = genGraph(seeds, accuProb, size1);
    }
    return graphs;
  }
   ...
   */
  public static Res genGraphDB(int Lv, int Le, int zB, int nB,
    ...
    LGraph[] A = genGraphs(B, accuProbB, zA, true, nA);
    //LGraph[] A=genBlocks(Lv,Le,k,r,true);

    // database graphs
    int nS = nB + nA;
    LGraph[] S = new LGraph[nS];
    System.arraycopy(B, 0, S, 0, nB);
    System.arraycopy(A, 0, S, nB, nA);
    double[] probS = new double[nS];
    double PB = 1 - PA;
    for(int i=0;i<nB;i++) {
      probS[i]=PB/nB;
    }
    ...
    double[] accuProbS = acuProb(probS);
    LGraph[] D = genGraphs(S, accuProbS, zG, true, nG);
  ...
  public static void main(String[] args) throws IOException {
    ...
    Res res = genGraphDB(Lv, Le, zB, nB, zA, k,
                              zG, nG, PA);
