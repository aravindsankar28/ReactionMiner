...
public static void main(String[] args) throws IOException, MathException, FeatureException, MoleculeIOException {
    ...
    if(opt.args()==2)
    {
	    argumentsDictionary.set(opt.getArg(0),opt.getArg(1),-1,-1,-1);
	    Hist[] background=Fingerprints.loadHistsFromFingerPrints(argumentsDictionary.graph);
	    DB = getSubset(background,argumentsDictionary.ids);
	    simProb = SimBasisProb.genBasisProb(background,2);
    }
    else
    {
	    argumentsDictionary.set(opt.getArg(0),opt.getArg(0),-1,-1,-1);
	    DB=Fingerprints.loadHistsFromFingerPrints(argumentsDictionary.graph);
	    simProb = SimBasisProb.genBasisProb(DB,2);
    }
    //DB=filter(DB,100, 3);
   ...
   // for(int i=0;i<DB.length;i++)
    ...
    Fingerprints.saveContainers(results,DB);
    Fingerprints.saveHists(results, sig_hist);
...
private static Hist[] getSubset(Hist[] background, String ids) {
	...
	System.out.println("Subset Size: "+DB.size());
