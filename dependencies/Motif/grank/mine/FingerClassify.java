		    	...
		    	for(int j=i+1;j<buf.size()&& !skip;j++)
		    	{
		    		if(i!=j)
		    		{
		    			...
		    			for(int k=0;k<h1.hist.length && (sub || sup);k++)
		    			{
		    				...
		    				if(h1.hist[k]<h2.hist[k])
		    					sup=false;
		    				if((h1.hist[k]!=0&& h2.hist[k]==0) || (h1.hist[k]==0&& h2.hist[k]!=0))
		    					close++;
		    			}
	    				if(sup || sub || close<=2)
	    				{
...
public static void main(String[] args) throws IOException, MathException, FeatureException, MoleculeIOException {
    ...
    double maxPvalue = opt.getDouble("pvalue", 1); //EDITED BY SAYAN
    
    int K = 100000;//opt.getInt("K", Integer.MAX_VALUE); EDITED BY SAYAN
    int hZ = 54;//opt.getInt("hZ", Integer.MAX_VALUE); EDITED BY SAYAN
    int hz =0;//opt.getInt("hz", 1); EDITED BY SAYAN
    ...
    for(int f=0;f<fold;f++)
    {
	    ...
	    background=Fingerprints.loadHistsFromFingerPrints(argumentsDictionary.graph);
	    System.out.println("Time spent on FingerPrints: "+(System.currentTimeMillis()-time0));
	    ...
	    {
	    	double[][] simProb = SimBasisProb.genBasisProb(background,40);
		    argumentsDictionary.set(opt.getArg(0),opt.getArg(1+classify),-1,-1,-1);
		    
		    DB = getSubset(background,argumentsDictionary.ids,classify,testSize,f,trainSize);
		    //results[classify]=new Hist[];
		   ...
		   // for(int i=0;i<DB.length;i++)
		    ...
		    results[classify] = mainProcess(maxPvalue, K, minSup, H, prob, simProb, hZ,
		                                 toEval, preEval, verbose);
		
		    if(classify==0)
		    	totPos+=results[classify].length;
		    else
		    	totNeg+=results[classify].length;
		...
		if(classify==1)
		{
		    ...
		    if (classify==2)
		    {
		    	...
		    	NNClassify(opt.getArg(1),opt.getArg(2),opt.getInt("k",5),testSize,f);
		    }
    }
}

private static Hist[] getSubset(Hist[] background, String ids, int val, int size, int fold, int totSize) {
	// TODO Auto-generated method stub
...
private static void NNClassify(String posFile, String negFile,int k,int size, int fold) {
	// TODO Auto-generated method stub
	...
	for(int j=0;j<labelSet.length;j++)
	{
		...
		PriorityQueue[] qs=calcClosest(labelSet[j],posResults,negResults,k);
		...
		{
			PriorityQueue p =qs[0];
			//p.add(new Cand(id,nodeScore));
			scores.put(id,p);
			PriorityQueue q =qs[1];
			//q.add(nodeScore[2]);
			posscores.put(id,q);
		}
...
private static int mode(PriorityQueue p,PriorityQueue q,int k) {
	...
	/*if(p.size()!=q.size())
	{
		System.out.println("HOW IS THIS POSSIBLE: P: "+p.size()+"\n+ q: "+q.size());
		new Scanner(System.in).next();
	}*/
...
private static PriorityQueue[] calcClosest(Hist hist, Hist[] posResults, Hist[] negResults, int k) {
	// TODO Auto-generated method stub
	int minNeg=Integer.MAX_VALUE;
	int id=-1;
	String graphid=hist.id.trim().split("-")[0];
	...
	for(int i=0;i<negResults.length;i++)
	{
		...
		Integer[] result={currSum,-1,1,i};
		p.add(new Cand(graphid,result));
	}		
	...
	PriorityQueue[] qs={p,q};//new PriorityQueue[2];
