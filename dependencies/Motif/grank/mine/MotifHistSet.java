    ...
    if (pvalue <= env.maxPvalue) {
      ...
      if (pvalue <= env.maxPvalue) {
        ...
        if (env.ans.size() > env.K) {
          ...
          if (env.verbose) {
            System.out.printf("pvalue=%g, sup=%d, z=%d\n",
                              env.maxPvalue, sup, z);
          }
        }
        cntUpdate++;
...
public static void main(String[] args) throws IOException, MathException, MoleculeIOException {
    ...
    if (opt.args() < 1) {
      ...
      System.out.println("  -pvalue=DOUBLE \t Maximum p-value, default=1");
      System.out.println("  -r \t restart probability in random walk");
      System.out.println("  -d \t convergence value in random walk");
      System.out.println("  -nta \t number of top atoms to count edges");
      System.out.println(
          "  -K=NUMBER \t\t Top-K significant subgraphs, default=MAX_INT");
      System.out.println("  -hZ=NUMBER \t\t Maximum sub-histogram size, default=maximum database histogram size");
      System.out.println(
          "  -hz=NUMBER \t\t Minimum sub-histogram size, default=1");
      System.out.println(
          "  -minSup=NUMBER[%] \t Minimum support, either number or percentage, default=1");
      System.out.println("  -mu0=[graph|hist] \t Use graphMu0 or histMu0 as the real support, default=graph");
      System.out.println(
          "  -sig_hist=FILE \t Significant histograms file, default=sig.hist");
      System.out.println(
          "  -eval=[yes|no] \t Whether to evaluate p-values, default=yes");
      System.out.println("  -preEval=[yes|no] \t Whether to compute the lower bound of p-value, default=yes");
      System.out.println("  -model=[complex|simple] \t default=complex");
    ...
    int K = 10000;//opt.getInt("K", Integer.MAX_VALUE); EDITED BY SAYAN
   ...
   // }
    for(int dbiter=0;dbiter<DB.length;dbiter++)
    {
        ...
        {
    	    simProb = SimBasisProb.genBasisProb(dbhist,11);
        }    
    	if(DB[dbiter]==null || DB[dbiter].size()==0)
    		continue;
    	//Hist[] dbhist=(Hist[])DB[dbiter].toArray(new Hist[1]);
	   ...
	   // for(int i=0;i<DB.length;i++)
	    ...
	    results[dbiter] = mainProcess(maxPvalue, K, minSup, H, prob, simProb, hZ,toEval, preEval, verbose);
	    //totalSize+=mainProcess(maxPvalue, K, minSup, H, prob, simProb, hZ, toEval, preEval, verbose).length;
	    System.out.printf(
		        "# of closed sub-vectors: %d, # of accurate pvalue: %d, # of top-K updates: %d\n",
		        cntEval, cntAccurate, cntUpdate);
		    //System.out.printf("# of future: %d\n", cntFuture);
		
		    // Format: minSupport% numFreqBin runtime cntEval cntAccurate cntUpdate cntResults
		    System.out.printf("%.1f %d %d %d %d %d\n", 100.0 * minSup / dbhist.length,
		                      numFreqBin, cntEval, cntAccurate,
		                      cntUpdate, results.length);
	    
    }
    //System.out.println("Num Hists: "+ totalSize);
    //new Scanner(System.in).next();
    String sig_hist = opt.getString("sig_hist", "sig.hist");
    //System.out.println(System.currentTimeMillis()-time0);
    Hist.saveContainers(results,DB,sig_hist);
    //Hist.saveHists(results, DB);
    long time1 = System.currentTimeMillis();
    double runtime = (time1 - time0) / 1000.0;
    System.out.printf("Time: %.2f sec\n", runtime);
  }

private static Vector[] getSubset(Vector[] background, String ids) {
	// TODO Auto-generated method stub
