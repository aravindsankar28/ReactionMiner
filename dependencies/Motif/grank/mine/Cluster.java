...
private static boolean[] booleanRep(double[] fv) {
	// TODO Auto-generated method stub
	boolean[] rep=new boolean[fv.length];
    ...
    for(int i=0;i<classList.length;i++)
    {
    	...
    	for(int j=0;j<classList[i].length;j++)
    	{
    		if(scores.get(classList[i][j].trim())==null)
    			continue;
    		file.append(classList[i][j].trim()+" ");
    		file.append(stringRep((double[])scores.get(classList[i][j].trim())));
    	}
    	FileUtils.writeToFile("bin.txt", classResults[i].length+"\n"+file.toString());
    	/*double[][] dist=new double[bin.length][bin.length];
    	for(int j=0;j<dist.length;j++)
    	{
    		file.append(classList[i][j].trim()+" ");
    		for(int k=j+1;k<dist.length;k++)
    		{
    			dist[j][k]=TanimotoMatrix.tanimotoDistance(bin[j],bin[k]);
    			dist[k][j]=dist[j][k];
    			file.append(dist[j][k]+" ");
    		}
