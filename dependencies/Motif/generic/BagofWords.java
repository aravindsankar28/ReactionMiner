...
public static void main(String[] args) throws IOException, MathException, MoleculeIOException {
	    ...
	    System.out.println("Pvalue: "+maxPvalue);
		    ...
		    for(int classify=0;classify<opt.args()-1;classify++)
		    {
			   ...
			   // }
			    ...
			    System.out.println("Saving significant hists");
			    String sig_hist = opt.getString("sig_hist", "sig.hist");
			    Hist.saveContainers(results[classify],DB,sig_hist);
		    }
    ...
    else if (opt.hasOpt("convertToVector"))
    {
    	...
    	convertToVector(argumentsDictionary.loadTopAtoms,opt.getArg(0));
    }
...
private static StringBuffer stringRep(String[] fv) {
	// TODO Auto-generated method stub
...
private static void convertToVector(String patterns, String fileName) throws FileNotFoundException
{
    ...
    for(int i=0;i<sigHists.length;i++)
    {
    		for(int b=0;b<labelSet.length;b++)
    		{
    			...
    			for(int l=0;l<labelSet[b].length;l++)
    			{
    				...
    				String dist=calcDistance(labelSet[b][l],sigHists[i]);
    				String[] currScore=(String[]) scores.get(id);
    				if(currScore!=null)
    				{	
    					if (dist!="False")
    					{
    ...
    for(int i=0;i<classList.length;i++)
    {
    	for(int j=0;j<classList[i].length;j++)
    	{
    		if(scores.get(classList[i][j].trim())==null)
    			continue;
    		count++;
    		classInfo.append(i+" "+classList[i][j].trim()+"\n");
    		file.append(stringRep((String[])scores.get(classList[i][j].trim())));
    	}
    }
    FileUtils.writeToFile(fileName+".bow", count+"\n"+file.toString());
    FileUtils.writeToFile(fileName+".info", classInfo.toString());
...
private static String calcDistance(Hist queryHist, Hist sigHist) {
	// TODO Auto-generated method stub
	double dist=0;
